...
module.exports = (function () {
  ...
  Route.prototype.storeRequest = function(req, res, next) {
    ...
    function storeRequest() {
      res.removeListener('finish', storeRequest);
      res.removeListener('close', storeRequest);
      // prams is an array with keys instead of index, so converts it to object
      var convertedParams = {};
      for (var name in req.params) {
        if (req.params.hasOwnProperty(name)) {
          convertedParams[name] = req.params[name];
        }
      }
    ...
    if (this.shouldStoreRequests) {
      res.on('finish', storeRequest);
      res.on('close', storeRequest);
    }
    next();
  };
